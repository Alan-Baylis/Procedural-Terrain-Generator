...
public class PerlinNoise
{
	...
	float FADE(float t) { return t * t * t * ( t * ( t * 6.0f - 15.0f ) + 10.0f ); } // ova fja vraca uglacane povrsi, ono njihovo fade-ovanje photoshop
	
	float LERP(float t, float a, float b) { return (a) + (t)*((b)-(a)); } // kao nasa linearna interpolacija 
	

	float GRAD2(int hash, float x, float y) // umesto naseg bitskog pomeranja, iako je sporije, menjanjem ove fje dobijaju se krivine
	{
	...
	float Noise2D( float x, float y )
	{
	    ...
	    //return ((0.507f * LERP( s, n0, n1 ))*2/3.0f)+0.3f;
	}
